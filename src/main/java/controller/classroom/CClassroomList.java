            ...
            if (result.isPresent()) {
                if (result.get() == ButtonType.OK) {
                    try {
                        ...
                        if(session.containsKey("subject") && session.containsKey("klass") && session.containsKey("lecture") && session.containsKey("lesson"))
                        {
                            ...
                            @NotNull final List<DBMLesson>  lessons         = MLesson.getAllMetadataFromClassroom(model, classroom, subjectMetadata, klassMetadata, lectureMetadata);
                            ...
                            lessonMetadata.addAll(MLesson.getAllMetadataFromSchool(model, this.schoolMetadata, subjectMetadata, klassMetadata, lectureMetadata));
                        }
            ...
            dialog.setScene(new Scene(IClassroomCreate.load(new CClassroomCreate(this.schoolMetadata, this.dayMetadata, this.periodMetadata, this.availabilities) {
                ...
                public void classroomCreated(@NotNull DBMClassroom classroom) {
                    try {
                        ...
                        CClassroomList.this.classroomList.setItems(FXCollections.observableList(CClassroomList.this.classroomMetadata));
