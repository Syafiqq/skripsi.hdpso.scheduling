...
public class CClassEditTimeOff implements Initializable {
    @NotNull
    ...
    public CClassEditTimeOff(@NotNull final DBClass klass, @NotNull final List<DBMDay> dayMetadata, @NotNull final List<DBMPeriod> periodMetadata, @NotNull final List<DBAvailability> availabilities) {
        this.klass = klass;
    ...
    public CClassEditTimeOff() throws UnsupportedEncodingException, SQLException {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        @NotNull final EventHandler<MouseEvent> tileClicked = event -> {
            if (event.getSource() instanceof CoordinatedLabel) {
                ...
                CClassEditTimeOff.this.processChanged(organizedLabel, label.getRow(), label.getColumn());
            }
        };
    ...
    private void changeTile(@NotNull final Label[][] labels, int c_day, int c_period) {
        @NotNull ObjectList<ObjectList<DBTimeOff>> timeOffs = this.klass.getTimeoff().getAvailabilities();
    ...
    public void onCloseClassEditTimeOffPressed(ActionEvent actionEvent) {
        ((Node) actionEvent.getSource()).getScene().getWindow().hide();
    }

    @SuppressWarnings("unchecked")
    public void onSaveClassEditTimeOffPressed(ActionEvent actionEvent) {
        try {
            @NotNull final AbstractModel model = new MClass(Setting.getDBUrl(Setting.defaultDB, DBType.DEFAULT));
            MClass.updateTimeOff(model, this.klass.getTimeoff());
            ...
            if (result.isPresent()) {
                if (result.get() == ButtonType.OK) {
                    this.timeOffUpdated(this.klass, this.klass.getTimeoff());
                    this.onCloseClassEditTimeOffPressed(actionEvent);
                }
            }
    ...
    public void timeOffUpdated(@NotNull final DBClass klass, @NotNull DBTimeOffContainer<DBClass> timeOff) {
