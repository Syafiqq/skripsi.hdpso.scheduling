...
public class CLectureEditTimeOff implements Initializable {
    @NotNull
    ...
    public CLectureEditTimeOff(@NotNull final DBLecture lecture, @NotNull final List<DBMDay> dayMetadata, @NotNull final List<DBMPeriod> periodMetadata, @NotNull final List<DBAvailability> availabilities) {
    ...
    public CLectureEditTimeOff() throws UnsupportedEncodingException, SQLException {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        @NotNull final EventHandler<MouseEvent> tileClicked = event -> {
            if (event.getSource() instanceof CoordinatedLabel) {
                ...
                controller.lecture.CLectureEditTimeOff.this.processChanged(organizedLabel, label.getRow(), label.getColumn());
            }
        };
    ...
    private void changeTile(@NotNull final Label[][] labels, int c_day, int c_period) {
        @NotNull ObjectList<ObjectList<DBTimeOff>> timeOffs = this.lecture.getTimeoff().getAvailabilities();
    ...
    public void onCloseLectureEditTimeOffPressed(ActionEvent actionEvent) {
        ((Node) actionEvent.getSource()).getScene().getWindow().hide();
    }

    @SuppressWarnings("unchecked")
    public void onSaveLectureEditTimeOffPressed(ActionEvent actionEvent) {
        try {
            @NotNull final AbstractModel model = new MLecture(Setting.getDBUrl(Setting.defaultDB, DBType.DEFAULT));
            MLecture.updateTimeOff(model, this.lecture.getTimeoff());
            ...
            if (result.isPresent()) {
                if (result.get() == ButtonType.OK) {
                    this.timeOffUpdated(this.lecture, this.lecture.getTimeoff());
                    this.onCloseLectureEditTimeOffPressed(actionEvent);
                }
            }
    ...
    public void timeOffUpdated(@NotNull final DBLecture lecture, @NotNull DBTimeOffContainer<DBLecture> timeOff) {
