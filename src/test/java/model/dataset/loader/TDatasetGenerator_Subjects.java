 ...
 * Name         : syafiq
 * Date / Time  : 03 November 2016, 9:45 AM.
 * Email        : syafiq.rezpector@gmail.com
 * Github       : syafiqq
 */
public class TDatasetGenerator_Subjects
{
    ...
    @Test public void classTest_A_001()
    {
        ...
        System.out.printf("[%3d] %s\n", dataset.getSubjects().length, Arrays.toString(Arrays.stream(dataset.getSubjects()).mapToInt(value -> value.getTimeoff().length).toArray()));
    }

    @Test public void classTest_B_001()
    {
        final @NotNull DatasetConverter encoder = this.dsLoader.getEncoder();
        System.out.println(encoder.getSubjects());
    }

    @Test public void classTest_C_001()
    {
        final @NotNull DatasetConverter decoder = this.dsLoader.getDecoder();
        System.out.println(decoder.getSubjects());
    }
    ...
    @Test public void classTest_D_001()
    {
        final @NotNull Dataset dataset = this.dsLoader.getDataset();
        int                    counter = -1;
        for(final DSTimeOff timeOff : dataset.getSubjects())
        {
